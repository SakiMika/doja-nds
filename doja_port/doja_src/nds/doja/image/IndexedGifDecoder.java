...
/** First-frame indexed GIF decoder used by DoJa PalettedImage. */
public final class IndexedGifDecoder {
    ...
    private IndexedGifDecoder(byte[] source) {
        data = source;
    }

    public static Result decode(byte[] source) throws IOException {
        if (!GifDecoder.isGif(source)) throw new IOException("not GIF");
        return new IndexedGifDecoder(source).decodeImage();
    }

    private Result decodeImage() throws IOException {
        pos = 6;
        width = readU16();
        height = readU16();
        int packed = readU8();
        ...
        while (pos < data.length) {
            int marker = readU8();
            if (marker == 0x21) readExtension();
            else if (marker == 0x2C) return readImageBlock();
            else if (marker == 0x3B) break;
            else throw new IOException("bad GIF block");
        }
    ...
    private void readExtension() throws IOException {
        ...
        if (label == 0xF9) {
            ...
            if (size == 4) {
                ...
                if (readU8() != 0) throw new IOException("bad GCE");
                return;
            }
            skipBytes(size);
        }
    ...
    private Result readImageBlock() throws IOException {
        int left = readU16();
        int top = readU16();
        ...
        if ((packed & 0x80) != 0) palette = readPalette(1 << ((packed & 7) + 1));
        if (palette == null || imageWidth <= 0 || imageHeight <= 0) throw new IOException("invalid GIF image");

        int minCodeSize = readU8();
        byte[] compressed = readSubBlocks();
        byte[] decoded = lzwDecode(compressed, minCodeSize, imageWidth * imageHeight);
        ...
        if (interlaced) {
            ...
            for (pass = 0; pass < 4; pass++) {
                int value;
                for (value = starts[pass]; value < imageHeight; value += steps[pass]) rowOrder[outRow++] = value;
            }
        }
        int y;
        for (y = 0; y < imageHeight; y++) {
            int row = interlaced ? rowOrder[y] : y;
            int dstY = top + row;
            if (dstY < 0 || dstY >= canvasHeight) continue;
            int x;
            for (x = 0; x < imageWidth; x++) {
                int dstX = left + x;
                if (dstX >= 0 && dstX < canvasWidth) canvas[dstY * canvasWidth + dstX] = decoded[y * imageWidth + x];
            }
        }
        Result result = new Result();
        result.width = canvasWidth;
        result.height = canvasHeight;
        result.indexes = canvas;
        result.palette = palette;
        result.transparentIndex = transparentIndex;
        return result;
    }

    private byte[] lzwDecode(byte[] input, int minimumCodeSize, int expected) throws IOException {
        if (minimumCodeSize < 2 || minimumCodeSize > 8) throw new IOException("bad LZW size");
        ...
        int datum = 0, bits = 0, inputPos = 0, outputPos = 0, first = 0, top = 0;
        int i;
        for (i = 0; i < clear; i++) suffix[i] = (byte)i;
    ...
    private int[] readPalette(int count) throws IOException {
        ...
        int i;
        for (i = 0; i < count; i++) {
            int red = readU8(), green = readU8(), blue = readU8();
    ...
    private void skipBytes(int count) throws IOException { require(count); pos += count; }
    private int readU8() throws IOException { require(1); return data[pos++] & 255; }
    private int readU16() throws IOException { int lo = readU8(); return lo | (readU8() << 8); }
    private void require(int count) throws IOException { if (count < 0 || pos + count > data.length) throw new IOException("truncated GIF"); }
}
