...
public class RmsGroup {
    ...
    /**
     * Used only when restoring persisted RMS data. MIDP RecordStore ids are
     * stable; after deleting id 1, ids 2 and 3 must not be compacted into
     * ids 1 and 2 on the next boot.
     */
    public int addRecordWithId(int recordId, byte[] data) {
        RmsRecord record = new RmsRecord(recordId, data);
        records.addElement(record);
        if (recordId >= recordCounter) {
            recordCounter = recordId + 1;
        }
        return record.getId();
    }
    ...
    public RmsRecord getRecordAt(int index) {
        if (index < 0 || index >= records.size()) {
            return null;
        }
        return (RmsRecord) records.elementAt(index);
    }
