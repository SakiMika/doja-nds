 ...
 * romized classes already carry their C function pointers.  DoJa protocol
 * classes, however, are loaded later from game.jar.  Their native methods
 * therefore need a small explicit bridge that remains active in a romized VM.
 *
 * Keep this list deliberately limited to the standalone ScratchPad protocol.
 */
#define DOJA_LATE_NATIVE_BIND 1
extern void Java_com_sun_cldc_io_j2me_scratchpad_Protocol_nativeSize(void);
extern void Java_com_sun_cldc_io_j2me_scratchpad_Protocol_nativeRead(void);
extern void Java_com_sun_cldc_io_j2me_scratchpad_Protocol_nativeReadBytes(void);
...
{
    const char *packageName = clazz->clazz.packageName == NULL
        ? "" : UStringInfo(clazz->clazz.packageName);
    ...
    if (strcmp(methodName, "nativeSize") == 0 &&
        xstrcmp(methodSignature, "()I") == 0) {
        result = Java_com_sun_cldc_io_j2me_scratchpad_Protocol_nativeSize;
    } else if (strcmp(methodName, "nativeRead") == 0 &&
               xstrcmp(methodSignature, "(I)I") == 0) {
        result = Java_com_sun_cldc_io_j2me_scratchpad_Protocol_nativeRead;
    } else if (strcmp(methodName, "nativeReadBytes") == 0 &&
               xstrcmp(methodSignature, "(I[BII)I") == 0) {
        result = Java_com_sun_cldc_io_j2me_scratchpad_Protocol_nativeReadBytes;
    } else if (strcmp(methodName, "nativeWrite") == 0 &&
               xstrcmp(methodSignature, "(II)V") == 0) {
        result = Java_com_sun_cldc_io_j2me_scratchpad_Protocol_nativeWrite;
    } else if (strcmp(methodName, "nativeWriteBytes") == 0 &&
               xstrcmp(methodSignature, "(I[BII)V") == 0) {
        result = Java_com_sun_cldc_io_j2me_scratchpad_Protocol_nativeWriteBytes;
    } else if (strcmp(methodName, "nativeFlush") == 0 &&
               xstrcmp(methodSignature, "()I") == 0) {
        result = Java_com_sun_cldc_io_j2me_scratchpad_Protocol_nativeFlush;
    }
