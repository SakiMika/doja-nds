...
loadExceptionHandlers(FILEPOINTER_HANDLE ClassFileH, METHOD_HANDLE thisMethodH)
{
    ...
    if (numberOfHandlers > 0) {
        ...
        /* quiet v14: printf("handlers: alloc ok ptr=%p\n", handlerTable); */
        ...
        ASSERTING_NO_ALLOCATION
            FOR_EACH_HANDLER(thisHandler, handlerTable)
                ...
                /* quiet v14: printf("handlers: item start=%u end=%u handler=%u ex=%u\n", startPC, endPC, handlerPC, exception); */
    ...
    printf("handlers: leave bytes=%u\n", (unsigned int)(numberOfHandlers * 8 + 2));
...
loadStackMaps(FILEPOINTER_HANDLE ClassFileH, METHOD_HANDLE thisMethodH)
{
    ...
    START_TEMPORARY_ROOTS
        ...
        /* quiet v21: printf("stackmap: alloc temp=%u ptrlist=%p\n", tempSize, stackMaps); */
        ...
        /* quiet v21: printf("stackmap: temp alloc ok ptr=%p\n", stackMap); */

        if (CurrentClass && CurrentClass->clazz.baseName && strcmp(UStringInfo(CurrentClass->clazz.baseName), "a") == 0) printf("stackmap: count=%u\n", nStackMaps);
        ...
        for (stackMapIndex = 0; stackMapIndex < nStackMaps; stackMapIndex++) {
             ...
             */
            thisMethod = unhand(thisMethodH);
            /* quiet v14: printf("stackmap: entry begin %u/%u\n", stackMapIndex, nStackMaps); */
            /* Read in the offset */
            stackMaps->data[stackMapIndex + nStackMaps].cell =
                loadShort(ClassFileH);
            /* quiet v14: printf("stackmap: entry offset=%ld\n", (long)stackMaps->data[stackMapIndex + nStackMaps].cell); */
            bytesRead += 2;
            for (index = 0, i = 0 ; i < 2; i++) {
                ...
                /* quiet v14: printf("stackmap: part=%u size=%u max=%u\n", i, size, maxSize); */
                bytesRead += 2;
                for (j = 0; j < size; j++) {
                    unsigned char stackType = loadByte(ClassFileH);
                    if (0) { } /* quiet v14: if (j < 8) printf("stackmap: type part=%u j=%u t=%u\n", i, j, stackType); */
                    bytesRead += 1;
            ...
            /* quiet v14: printf("stackmap: parts done index=%u\n", index); */
            ...
            /* quiet v14: printf("stackmap: entry ok %u\n", stackMapIndex); */
        }
    END_TEMPORARY_ROOTS
    if (CurrentClass && CurrentClass->clazz.baseName && strcmp(UStringInfo(CurrentClass->clazz.baseName), "a") == 0) printf("stackmap: leave bytes=%ld\n", bytesRead);
    return bytesRead;
}
                  ...
                  POINTERLIST_HANDLE StringPoolH)
{
    ...
    if (thisMethod->ofClass && thisMethod->ofClass->clazz.baseName && strcmp(UStringInfo(thisMethod->ofClass->clazz.baseName), "a") == 0) printf("code: header maxStack=%u frame=%u len=%lu\n", thisMethod->u.java.maxStack, thisMethod->frameSize, (unsigned long)codeLength);
    ...
    if (thisMethod->ofClass && thisMethod->ofClass->clazz.baseName && strcmp(UStringInfo(thisMethod->ofClass->clazz.baseName), "a") == 0) printf("code: alloc begin cells=%lu\n", (unsigned long)ByteSizeToCellSize(codeLength));
    ...
    if (thisMethod->ofClass && thisMethod->ofClass->clazz.baseName && strcmp(UStringInfo(thisMethod->ofClass->clazz.baseName), "a") == 0) printf("code: alloc ok ptr=%p\n", code);
    thisMethod = unhand(thisMethodH);
    thisMethod->u.java.code = code;

    thisMethod->u.java.codeLength = codeLength;
    if (thisMethod->ofClass && thisMethod->ofClass->clazz.baseName && strcmp(UStringInfo(thisMethod->ofClass->clazz.baseName), "a") == 0) printf("code: load bytes begin len=%lu\n", (unsigned long)codeLength);
    loadBytes(ClassFileH, (char *)thisMethod->u.java.code, codeLength);
    if (thisMethod->ofClass && thisMethod->ofClass->clazz.baseName && strcmp(UStringInfo(thisMethod->ofClass->clazz.baseName), "a") == 0) printf("code: load bytes ok\n");
    actualAttrLength = 2 + 2 + 4 + codeLength;

    /* Load exception handlers associated with the method */
    if (thisMethod->ofClass && thisMethod->ofClass->clazz.baseName && strcmp(UStringInfo(thisMethod->ofClass->clazz.baseName), "a") == 0) printf("code: handlers begin\n");
    actualAttrLength += loadExceptionHandlers(ClassFileH, thisMethodH);
    if (thisMethod->ofClass && thisMethod->ofClass->clazz.baseName && strcmp(UStringInfo(thisMethod->ofClass->clazz.baseName), "a") == 0) printf("code: handlers ok\n");

    nCodeAttrs = loadShort(ClassFileH);
    if (thisMethod->ofClass && thisMethod->ofClass->clazz.baseName && strcmp(UStringInfo(thisMethod->ofClass->clazz.baseName), "a") == 0) printf("code: attr count=%d\n", nCodeAttrs);
    actualAttrLength += 2;
    for (codeAttrIndex = 0; codeAttrIndex < nCodeAttrs; codeAttrIndex++) {
        ...
        if (thisMethod->ofClass && thisMethod->ofClass->clazz.baseName && strcmp(UStringInfo(thisMethod->ofClass->clazz.baseName), "a") == 0) printf("code attr: index=%d name=%s len=%lu\n", codeAttrIndex, codeAttrName, (unsigned long)codeAttrLength);
        ...
        if (!strcmp(codeAttrName, "StackMap")) {
            ...
            needStackMap = FALSE;
            if (thisMethod->ofClass && thisMethod->ofClass->clazz.baseName && strcmp(UStringInfo(thisMethod->ofClass->clazz.baseName), "a") == 0) printf("code attr: stackmap begin len=%lu\n", (unsigned long)codeAttrLength);
            stackMapAttrSize = loadStackMaps(ClassFileH, thisMethodH);
            if (thisMethod->ofClass && thisMethod->ofClass->clazz.baseName && strcmp(UStringInfo(thisMethod->ofClass->clazz.baseName), "a") == 0) printf("code attr: stackmap ok actual=%lu expect=%lu\n", (unsigned long)stackMapAttrSize, (unsigned long)codeAttrLength);
            if (stackMapAttrSize != codeAttrLength) {
                raiseExceptionWithMessage(ClassFormatError,
                    KVM_MSG_BAD_ATTRIBUTE_SIZE);
            }
        } else {
            /* quiet v14: printf("code attr: skip begin %s len=%lu\n", codeAttrName, (unsigned long)codeAttrLength); */
            skipBytes(ClassFileH, codeAttrLength);
            /* quiet v14: printf("code attr: skip ok %s\n", codeAttrName); */
        }
        actualAttrLength += 6 + codeAttrLength;
    }
    if (thisMethod->ofClass && thisMethod->ofClass->clazz.baseName && strcmp(UStringInfo(thisMethod->ofClass->clazz.baseName), "a") == 0) printf("code: leave actual=%lu\n", (unsigned long)actualAttrLength);
                     ...
                     POINTERLIST_HANDLE StringPoolH)
{
    ...
    if (thisMethod->ofClass && thisMethod->ofClass->clazz.baseName && strcmp(UStringInfo(thisMethod->ofClass->clazz.baseName), "a") == 0) printf("method attrs: count=%u needCode=%d\n", attrCount, needCode);
    /* See if the field has any attributes in the class file */
    for (attrIndex = 0; attrIndex < attrCount; attrIndex++) {
        unsigned short attrNameIndex = loadShort(ClassFileH);
        unsigned int   attrLength    = loadCell(ClassFileH);
        char*          attrName      = getUTF8String(StringPoolH, attrNameIndex);
        if (thisMethod->ofClass && thisMethod->ofClass->clazz.baseName && strcmp(UStringInfo(thisMethod->ofClass->clazz.baseName), "a") == 0) printf("method attrs: index=%d name=%s len=%lu\n", attrIndex, attrName, (unsigned long)attrLength);
        ...
        if (strcmp(attrName, "Code") == 0) {
            ...
            if (thisMethod->ofClass && thisMethod->ofClass->clazz.baseName && strcmp(UStringInfo(thisMethod->ofClass->clazz.baseName), "a") == 0) printf("method attrs: code begin len=%lu\n", (unsigned long)attrLength);
            actualLength = loadCodeAttribute(ClassFileH, thisMethodH, StringPoolH);
            if (thisMethod->ofClass && thisMethod->ofClass->clazz.baseName && strcmp(UStringInfo(thisMethod->ofClass->clazz.baseName), "a") == 0) printf("method attrs: code ok actual=%lu expect=%lu\n", (unsigned long)actualLength, (unsigned long)attrLength);
            ...
            /* quiet v14: printf("method attrs: exceptions count=%lu\n", (unsigned long)exceptionCount); */
            ...
            /* quiet v14: printf("method attrs: skip %s len=%lu\n", attrName, (unsigned long)attrLength); */
            skipBytes(ClassFileH, attrLength);
            /* quiet v14: printf("method attrs: skip ok %s\n", attrName); */
        }
    }
    /* quiet v14: printf("method attrs: leave needCode=%d\n", needCode); */
    if (needCode) {
        raiseExceptionWithMessage(ClassFormatError,
...
static int
pstros_method_trace_small(const char *className, unsigned short methodCount, unsigned int index)
{
    if (className == NULL) return 1;
    if (strcmp(className, "a") == 0) return 1;
    if (strcmp(className, "GloftDIRU") == 0) return 1;
    if (methodCount <= 20) return 1;
    if ((index % 25) == 0) return 1;
    return 0;
}
    ...
    START_TEMPORARY_ROOTS
        ...
        if (CurrentClass->clazz.baseName) {
            ...
            if (strcmp(cn, "a") == 0 || strcmp(cn, "GloftDIRU") == 0) {
                printf("method: name=%s sig=%s class=%s\n", methodName, signature, cn);
            }
        }
        ...
        /* quiet v14: printf("method: flags ok %s\n", methodName); */
        verifyName(methodName, LegalMethod);
        /* quiet v14: printf("method: name ok %s\n", methodName); */

        result.nt.nameKey =
            change_Name_to_Key(&methodName, 0, strlen(methodName));
        /* quiet v14: printf("method: name key ok %s\n", methodName); */
        result.nt.typeKey =
            change_MethodSignature_to_Key(&signature, 0, strlen(signature));
        /* quiet v14: printf("method: sig key ok %s\n", methodName); */
        ...
        if (thisMethod->ofClass && thisMethod->ofClass->clazz.baseName && strcmp(UStringInfo(thisMethod->ofClass->clazz.baseName), "a") == 0) printf("method: attrs begin %s\n", methodName);
        loadMethodAttributes(ClassFileH, thisMethodH, StringPoolH);
        if (thisMethod->ofClass && thisMethod->ofClass->clazz.baseName && strcmp(UStringInfo(thisMethod->ofClass->clazz.baseName), "a") == 0) printf("method: attrs ok %s\n", methodName);
        ...
        if (accessFlags & ACC_NATIVE) {
            ...
            /* quiet v14: printf("method: native lookup begin %s %s\n", methodName, signature); */
            thisMethod->u.native.info = NULL;
            thisMethod->u.native.code =
                getNativeFunction(CurrentClass, methodName, signature);
            /* quiet v14: printf("method: native lookup ok %s code=%p\n", methodName, thisMethod->u.native.code); */
            ...
            POINTERLIST_HANDLE StringPoolH)
{
    ...
    printf("methods: count=%u class=%s\n", methodCount, dbgClassName);
    if (methodCount == 0) {
        return;
    }
    START_TEMPORARY_ROOTS
...
#endif
        printf("methods: alloc ok cells=%d ptr=%p class=%s\n", tableSize, methodTable, dbgClassName);
        ...
        for (index = 0; index < methodCount; index++) {
...
#endif
            if (pstros_method_trace_small(dbgClassName, methodCount, index)) printf("methods: begin index=%u/%u class=%s\n", index, methodCount, dbgClassName);
#if USESTATIC
            START_TEMPORARY_ROOTS
            ...
            loadOneMethod(ClassFileH, CurrentClass, &thisMethod, StringPoolH);
#endif
            if (pstros_method_trace_small(dbgClassName, methodCount, index)) printf("methods: ok index=%u class=%s\n", index, dbgClassName);
        }
    END_TEMPORARY_ROOTS

    /* quiet v14: printf("methods: duplicate check class=%s\n", dbgClassName); */
    if (methodCount >= 2) {
...
loadClassfile(INSTANCE_CLASS InitiatingClass, bool_t fatalErrorIfFail)
{
    ...
    TRY {
        ...
        while (clazz != NULL && clazz->status == CLASS_RAW) {
            ...
            printf("load raw done: %s status=%d\n", getClassName((CLASS)clazz), clazz->status);
