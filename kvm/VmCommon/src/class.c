...
//Torlus
#ifdef DISABLE_VERIFIER
            /*
             * The class-file StackMap attribute is loaded in verifier format.
             * Normally verifyClass() rewrites every verifier map into the
             * compact pointer bitmap consumed by the exact garbage collector.
             *
             * Pstros omits the verifier to save space, but the old shortcut
             * merely changed the class state to CLASS_VERIFIED.  As a result,
             * getGCRegisterMask() interpreted a POINTERLIST verifier map as a
             * STACKMAP pointer map and eventually aborted with
             * "Illegal stack size" as soon as GC ran inside a class that had
             * a StackMap attribute.
             *
             * Verification may be disabled, but the GC conversion is still
             * mandatory.  Perform the same final rewrite that the verifier
             * normally performs before allowing the class to execute.
             */
            if (thisClass->methodTable != NULL) {
                int i;
            ...
            setClassStatus(thisClass, CLASS_VERIFIED);
#else
...
runClinitException(FRAME_HANDLE frameH)
{
    START_TEMPORARY_ROOTS
        ...
        if (!isAssignableTo((CLASS)(exception->ofClass),(CLASS)JavaLangError)) {
             ...
             *   Static initializer in <className>: <className of throwable> <message>
             */
            strcpy(p, "Static initializer in ");
            p += strlen(p);
            getClassName_inBuffer((CLASS)thisClass, p);
            p += strlen(p);
            strcpy(p, ": ");
            p += strlen(p);
