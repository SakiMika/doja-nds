...
void InitializeHeap(void)
{
    ...
    printf("heap init: requested=%ld compaction=%d\n", (long)RequestedHeapSize, (int)ENABLE_HEAP_COMPACTION);
...
/* Remember: size is given in CELLs rather than bytes */
{
    ...
    if (thisChunk == NULL) {
        ...
        if (thisChunk == NULL) {
            CHUNK scanChunk;
            long largestFreeCells = 0;
            long totalFreeBytes = memoryFree();
            for (scanChunk = FirstFreeChunk; scanChunk != NULL;
                 scanChunk = scanChunk->next) {
                long chunkCells = SIZE(scanChunk->size) + HEADERSIZE;
                if (chunkCells > largestFreeCells) {
                    largestFreeCells = chunkCells;
                }
            }
            printf("KVM HEAP OOM req=%ld free=%ld largest=%ld\n",
                   realSize * CELL, totalFreeBytes, largestFreeCells * CELL);
            return NULL;
        }
    }
