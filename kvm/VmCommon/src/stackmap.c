...
skipEncodedMethodType(unsigned char *from);
...
again:
        ...
        switch(token) {
            ...
            case WIDE:
                ...
                switch (token) {
                    ...
                    case IINC:
                        thisIP += 2; break;
                    case RET:
                        /* RET has no stack effect for GC pointer maps. */
                        break;
                    default:
                        printf("stackmap: unexpected WIDE opcode=%d pc=%ld\n",
                               token, (long)((thisIP - 4) - code));
                        fatalError(KVM_MSG_UNEXPECTED_BYTECODE);
                        break;
                }
                break;

            /* Bytecodes that the original KVM stack map walker did not
             * need on some ports, but that appear in real obfuscated
             * MIDP games or in KVM's own patched methods.  They are safe
             * to model here because we only need the stack pointer/pointer
             * bitmap, not the actual branch target.
             */
            case GOTO:
                thisIP += 2;
                break;
            ...
            case JSR:
                thisIP += 2;
                goto pushInt;       /* returnAddress is not a pointer */
            ...
            case TABLESWITCH: {
                long low;
                long high;
                while (((thisIP - code) & 3) != 0) {
                    thisIP++;
                }
                /* default */
                thisIP += 4;
                low = ((long)thisIP[0] << 24) | ((long)thisIP[1] << 16) |
                      ((long)thisIP[2] << 8)  | (long)thisIP[3];
                thisIP += 4;
                high = ((long)thisIP[0] << 24) | ((long)thisIP[1] << 16) |
                       ((long)thisIP[2] << 8)  | (long)thisIP[3];
                thisIP += 4;
                if (high >= low) {
                    thisIP += 4 * (high - low + 1);
            ...
            case LOOKUPSWITCH: {
                ...
                /* default */
                thisIP += 4;
                npairs = ((long)thisIP[0] << 24) | ((long)thisIP[1] << 16) |
                         ((long)thisIP[2] << 8)  | (long)thisIP[3];
                thisIP += 4;
                if (npairs > 0) {
                    thisIP += 8 * npairs;
            ...
            case CUSTOMCODE:
                /* java.lang.Class.runCustomCode() is patched to this
                 * internal bytecode by KVM.  GC can ask for its register
                 * map while the callback is running.  It has no Java stack
                 * effect that needs to be simulated here.
                 */
                break;
            ...
            case INVOKEINTERFACE_FAST:
            {
                ...
                methodArgCount = method->argCount;
            ...
            case INVOKEINTERFACE: {
                ...
                if (CONSTANTPOOL_TAG(cp, cpIndex) & CP_CACHEBIT) {
                    ...
                    method = NULL;
                    getUnresolvedMethodStackInfo(cp, cpIndex, token,
                                                 &methodArgCount,
                                                 &methodAccessFlags);
                }
           }
            ...
            default:
                printf("stackmap: unexpected bytecode token=0x%02x pc=%ld target=%ld class=%s\n",
                       token,
                       (long)((thisIP - 1) - code),
                       (long)(targetIP - code),
                       thisMethod->ofClass->clazz.baseName ?
                           UStringInfo(thisMethod->ofClass->clazz.baseName) : "?");
                fatalError(KVM_MSG_UNEXPECTED_BYTECODE);
        }
...
getUnresolvedMethodStackInfo(CONSTANTPOOL cp, unsigned int cpIndex,
                             ...
                             long *accessFlagsP)
{
    CONSTANTPOOL_ENTRY methodEntry = &cp->entries[cpIndex];
    unsigned short nameTypeIndex = methodEntry->method.nameTypeIndex;
    NameTypeKey nameTypeKey = cp->entries[nameTypeIndex].nameTypeKey;
    unsigned char *from = (unsigned char *)
        change_Key_to_Name(nameTypeKey.nt.typeKey, NULL);
    unsigned int argCount = *from++;
    unsigned int i;
    unsigned char returnTag;

    for (i = 0; i < argCount; i++) {
        from = skipEncodedMethodType(from);
    }

    if (token != INVOKESTATIC
#if ENABLEFASTBYTECODES
        && token != INVOKESTATIC_FAST
#endif
       ) {
        argCount++;
    }
    ...
    *argCountP = argCount;
    ...
    if (returnTag == 'V') {
        *accessFlagsP = ACC_POINTER | ACC_DOUBLE;
    } else if (returnTag == 'J' || returnTag == 'D') {
        *accessFlagsP = ACC_DOUBLE;
    } else if (returnTag == 'L') {
        *accessFlagsP = ACC_POINTER;
    } else if (returnTag >= 'A' && returnTag <= 'Z') {
        *accessFlagsP = 0;
    } else {
        *accessFlagsP = ACC_POINTER;
    }
