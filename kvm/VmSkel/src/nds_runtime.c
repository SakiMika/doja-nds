...
cell *allocateHeap(long *sizeptr, void **realresultptr) {
    ...
    /* v42 asks for an 8 MiB Java heap in DSi mode.  Leave a controlled
    ...
    *realresultptr = space;
    if (space == NULL) {
        printf("DoJa v42 heap malloc failed: requested=%ld minimum=%ld\n",
               requested, (long)DEFAULTHEAPSIZE);
        return NULL;
    }
    *sizeptr = attempt;
    if (attempt != requested) {
        printf("DoJa v42 heap fallback: requested=%ld actual=%ld\n",
               requested, attempt);
    }
    printf("DoJa v42 heap allocated: %ld bytes\n", attempt);
