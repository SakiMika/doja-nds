...
#include <stdio.h>
#include <jar.h>
#include <pstros_embedded_resources.h>
...
#define FILE_KIND_STDIO 0
#define FILE_KIND_JAR   1
#define FILE_KIND_ROM   2
...
struct jarPointerStruct {
    bool_t isJarFile;      /* FILE_KIND_JAR */
    long dataLen;          /* length of data stream */
    long dataIndex;        /* current position for reading */
    unsigned char data[1];
};

struct romPointerStruct {
    bool_t isJarFile;      /* FILE_KIND_ROM */
    long dataLen;
    long dataIndex;
...
static FILEPOINTER
openFilesystemFile(BYTES fileName) {
    FILE *file;
    FILEPOINTER fp;
    long filesize;
#ifndef POCKETPC
    ...
    fp = (FILEPOINTER)mallocBytes(sizeof(struct stdioPointerStruct));
    fp->isJarFile = FILE_KIND_STDIO;
...
static FILEPOINTER
openEmbeddedResource(BYTES fileName) {
    struct jarPointerStruct *result;
    long length;
    ...
    if (strcmp(fileName, "nds/pstros/video/system.dsf") != 0 &&
        strcmp(fileName, "/nds/pstros/video/system.dsf") != 0 &&
        strcmp(fileName, "system.dsf") != 0) {
        return NULL;
    }

    length = (long)pstros_system_dsf_len;
    result = (struct jarPointerStruct*)mallocBytes(
        offsetof(struct jarPointerStruct, data[0]) + length);
    if (result == NULL) {
        return NULL;
    }

    result->isJarFile = FILE_KIND_JAR;
    result->dataLen = length;
    result->dataIndex = 0;
    memcpy(result->data, pstros_system_dsf, length);
    printf("embedded resource: system.dsf len=%ld\n", length);
    return (FILEPOINTER)result;
}
...
openResourcefile(BYTES resourceNameArg) {
    ...
    START_TEMPORARY_ROOTS
        ...
        ClassFile = openEmbeddedResource(resourceName);
        if (ClassFile == NULL) {
            ClassFile = openClassfileInternal(&resourceName);
        }
        if (ClassFile == NULL) {
            ClassFile = openFilesystemFile(resourceName);
        }
        if (ClassFile == NULL) {
#if INCLUDEDEBUGCODE
            if (traceclassloadingverbose) {
...
openClassfileInternal(BYTES_HANDLE filenameH) {
    printf("openClassfileInternal: %s\n", unhand(filenameH));
...
loadByteNoEOFCheck(FILEPOINTER_HANDLE ClassFileH)
{
    ...
    if (ClassFile->isJarFile == FILE_KIND_STDIO) {
        FILE *file = ((struct stdioPointerStruct*)ClassFile)->file;
        return getc(file);
    } else if (ClassFile->isJarFile == FILE_KIND_ROM) {
        struct romPointerStruct *ds = (struct romPointerStruct *)ClassFile;
    ...
    int length, bool_t checkEOF)
{
    ...
    if (ClassFile->isJarFile == FILE_KIND_STDIO) {
        ...
        if (pos == -1)
            pos = ds->dataIndex;
        avail = (int)(ds->dataLen - pos);
        if (avail < length && checkEOF) {
            raiseExceptionWithMessage(ClassFormatError,
                KVM_MSG_CLASSFILE_SIZE_DOES_NOT_MATCH);
            return 0;
        }
        if (avail <= 0)
            return -1;
        readLength = avail > length ? length : avail;
        ...
        struct jarPointerStruct *ds = (struct jarPointerStruct *)ClassFile;
        int avail;
        if (ds->dataLen == (long)pstros_system_dsf_len) {
            printf("dsf read: pos=%d len=%d idx=%ld\n", pos, length, ds->dataIndex);
        }
        if (pos == -1)
            pos = ds->dataIndex;
...
skipBytes(FILEPOINTER_HANDLE ClassFileH, unsigned long length)
{
    ...
    if (ClassFile->isJarFile == FILE_KIND_STDIO) {
        ...
        } else if (pos + length > (unsigned int)ds->size) {
            raiseExceptionWithMessage(ClassFormatError,
                KVM_MSG_CLASSFILE_SIZE_DOES_NOT_MATCH);
        } else {
        ...
        struct romPointerStruct *ds = (struct romPointerStruct *)ClassFile;
        unsigned long avail = ds->dataLen - ds->dataIndex;
        if (avail < length) {
            raiseExceptionWithMessage(ClassFormatError,
                KVM_MSG_CLASSFILE_SIZE_DOES_NOT_MATCH);
        } else {
...
void closeClassfile(FILEPOINTER_HANDLE ClassFileH)
{
    ...
    /* JAR and ROM-backed resources need no close action. */
}
...
#if !JAR_FILES_USE_STDIO
           if (strcmp(result->name, "@embedded") == 0) {
               long embeddedLength = (long)(
                   _binary_embedded_game_jar_end -
                   _binary_embedded_game_jar_start);
               if (embeddedLength > 0 &&
                   openJARFile((void *)_binary_embedded_game_jar_start,
                               embeddedLength, &result->u.jarInfo)) {
                   result->type = 'j';
               }
           } else
#endif
...
#else
           /* This converts result->name to WCHAR for directory checks. */
