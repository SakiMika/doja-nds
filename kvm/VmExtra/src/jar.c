...
static int jar_getBytes(unsigned char*, int, void* p);
...
#if !JAR_FILES_USE_STDIO
/*
 * Memory-backed JAR input used by the inflater.  The modern inflater asks
 * the callback to fill a buffer, whereas the old NDS port returned only one
 * byte and never wrote to that buffer.  That made every DEFLATED class fail
 * with inflateData() == FALSE even though the central-directory lookup had
 * succeeded.
 */
typedef struct memoryJarInputStruct {
    ...
    switch (method) { 
        ...
        case DEFLATED: {
            ...
            START_TEMPORARY_ROOTS
                ...
                void *arg = file;
#else
                MemoryJarInput memoryInput;
                void *arg;

                memoryInput.cursor = p;
 ...
 */
static int
jar_getBytes(unsigned char *buff, int length, void* p) {
#if JAR_FILES_USE_STDIO
    return fread(buff, sizeof(char), length, (FILE *)p);
#else
    MemoryJarInput *input = (MemoryJarInput *)p;
    int count;

    if (input == NULL || buff == NULL || length <= 0 ||
        input->remaining <= 0) {
        return 0;
    }
    ...
    memcpy(buff, input->cursor, count);
    input->cursor += count;
